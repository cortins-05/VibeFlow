...
- `services/` — db (SQLite), youtube (API + streaming), trackPlayerService
...
1. Share fix: `Share.share({url})` → `expo-sharing` `shareAsync` in downloads
2. Removed visualizer tap toggle: artwork ↔ lyrics direct toggle (no visualizer mode)
3. Swipe right → add to queue on ALL TrackRows (home, search, favorites, history, downloads, playlist)
4. Swipe left → remove from playlist (only on playlist screen, no confirmation dialog)
5. TrackActionsModal enlarged: bigger padding, fonts, hit areas
...
- Release APK build: `./gradlew app:createBundleReleaseJsAndAssets app:assembleRelease` from android/
